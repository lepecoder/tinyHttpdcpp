...
#include <string>
#include <strings.h>    // 基于byte的一些函数
#include <sys/socket.h> // 调用socket接口，socket是应用层与TCP/IP通信的中间软件抽象层
...
int main(void) {
    int server_sock = -1;           // 这是服务套接字的文件描述符
    u_short port = 0;               // 端口号
    int client_sock = -1;           // 客户通信套接字的文件描述符
    struct sockaddr_in client_name; // 客户端套接字结构
